...
All components have been soldered by hand using pneumatic solder paste dispensers, placed with a pair of tweezers, and reflowed using a $\SI{250}{\celsius}$ heating plate. This process is not difficult, except for the display connector and the non-\gls{lqfp} components, which require more concentration and time.
...
\begin{itemize}
    ...
    \item \lstinline{task_mode_tick} is the main loop that runs a procedure based on the current dive / surface mode
    ...
    \item \lstinline{task_bluetooth_button} (unused in V1)
\end{itemize}
...
All dynamic scheduling rate algorithms in this work are based on the \gls{aimd} principle, the basic \lstinline{AimdRateAlgorithm} has a number of milliseconds for additive increase, a fraction (numerator, denominator) for multiplicative decrease, and a maximum \gls{tbe} that should not be exceeded \ref{lst:aimd}.
...
We first calculate the change rate per second $r$, then transform it into a number $f$ between $0$ and $1$ using a fixed reference speed $ref$ (\ref{align:diff_aimd_f}).
\begin{align}
    r &= (v_n - v_0) / (t_n - t_0) \label{align:diff_aimd_r} \\
    f &= r / ref \label{align:diff_aimd_f}
\end{align}
