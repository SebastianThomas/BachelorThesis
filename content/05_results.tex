...
The variable rate algorithms are on average $7.4\times$ more expensive, $1858 \pm 32$ than the fixed-rate algorithm and $6.98\times$ more expensive than a \gls{o2tox} calculation.

\input{./content/generated/05_results_timing_o2tox}
\input{./content/generated/05_results_timing_o2tox_rate}

Compared to the fixed rate baseline, the \gls{o2tox} algorithm is executed $0.65\times$ and $0.82\times$ as often on the $\SI{50}{\meter}$ profile, $0.15\times$ and $0.25\times$ as often on the $\SI{90}{\meter}$ profile, depending on the specific decompression profile. For the shallow dive to $\SI{20}{\meter}$, no execution is required following the variable rate algorithm.
...
Compared to the baseline execution count, depending on the profile, the variable rate algorithm leads to a $1.17\times$ to $6\times$ decrease in the execution count, which is in an almost linear relationship to the total cycle count spent on the computation of the decompression schedule, while only incurring a $0.5\%$ to $1.8\%$ overhead compared to the execution of a decompression schedule computation.
